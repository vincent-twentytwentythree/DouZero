...
class LandlordLstmModel(nn.Module):
    ...
    def forward(self, z, x, return_value=False, flags=None, topk=None):
        ...
        if return_value:
            ...
            values, indices = torch.topk(x, k=min(topk, x.shape[0]), dim=0)
...
class GeneralModel(nn.Module):
    def __init__(self):
        ...
        self.in_planes = 80
        #input 1*54*20
        self.conv1 = nn.Conv1d(20, 80, kernel_size=(3,),
        ...
        self.linear1 = nn.Linear(320 * BasicBlock.expansion * 3 + 17 * 4, 1024) # MYWEN
...
model_dict['pk_dp'] = LandlordLstmModel
...
class Model:
    ...
    def __init__(self, device=0, training_mode=None):
        ...
        if training_mode == None:
            self.models['landlord'] = GeneralModel().to(device)
            self.models['second_hand'] = GeneralModel().to(device)
        if training_mode != None:
            self.models[training_mode] = GeneralModel().to(device)
